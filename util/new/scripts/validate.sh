...
COMPAREDIR=/project/archive-index-data/seiler/kmer_RefSeq/compare_mantis_lemma_normal
...
IFS=: read readID binID <<< $(head -n 1 ${COMPAREDIR}/raptor.fns)

# Extract accession from string, e.g. `2164    GCF_000332755.1_ASM33275v1_genomic`. m 1 => Stop after 1 match
accession=$(grep -m 1 "^${binID}" ${COMPAREDIR}/user_bins.ids | cut -f 2)
