...
READ_ERRORS=2
READ_LENGTH=250
# Seed used for simulate_sequence
SEED=0
THREADS=12
...
check_tools() {
    local tools=("awk" "numfmt" "wget" "tar")
...
validate_variables() {
    ...
    for var_name in REFERENCE_LENGTH NUMBER_OF_HAPLOTYPES READ_COUNT READ_LENGTH THREADS; do
        var_value=${!var_name}
        if [[ "$var_value" -le 0 ]]; then
            echo "[ERROR] $var_name must be greater than 0. Current value: $var_value"
            return 1
        fi
    done

    for var_name in SEED READ_ERRORS; do
        var_value=${!var_name}
        if [[ "$var_value" -lt 0 ]]; then
            echo "[ERROR] $var_name must be greater than or equal to 0. Current value: $var_value"
            return 1
        fi
    done
}
...
check_config() {
    ...
    if [[ -n "${REPO_PATH}" && ! -d "${REPO_PATH}" ]]; then
        echo "[ERROR] The given REPO_PATH does not exist: '${REPO_PATH}'"
        return 1
    fi

    if [[ -n "${SAMPLE_FROM}" ]]; then
        if [[ ! -f "${SAMPLE_FROM}" ]]; then
            echo "[ERROR] The given SAMPLE_FROM does not exist: '${SAMPLE_FROM}'"
            return 1
        else
            awk -v filename="${SAMPLE_FROM}" '{
                if ($0 !~ /^(|[0-9].*)$/) {
                    print "[ERROR] \"" filename "\" starts with non-integer on line " NR ": \"" $0 "\""
                    found_offending = 1
                }
                } END {
                    if (found_offending) {
    ...
    for BINS in ${NUMBER_OF_BINS}; do
        ...
        if [[ "${BINS}" -le 0 ]]; then
            echo "[ERROR] NUMBER_OF_BINS must be greater than 0. Current value: ${BINS}"
            return 1
        fi
...
# For shellcheck SC2034 (apparently unused variables)
: "${SEED}"
: "${THREADS}"
