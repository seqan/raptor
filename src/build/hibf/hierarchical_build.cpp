                    ...
                    bool is_root)
{
    ...
    auto && ibf = construct_ibf(parent_kmers, kmers, max_bin_tbs, current_node, data, config, is_root);
    kmers.clear(); // reduce memory peak

    // parse all other children (merged bins) of the current ibf
    loop_over_children(parent_kmers, ibf, ibf_positions, current_node, data, config, is_root);
