...
TEST_F(build_hibf, pipeline_from_chopper)
{
    ...
    { // generate count file
        ...
        seqan3::argument_parser parser{"chopper-count", static_cast<int>(strlen(*argv)), argv, seqan3::update_notifications::off};
        chopper::count::execute(parser);
    }

    { // generate layouy file
        const char * argv[] = {"./chopper-layout",
                               "--technical-bins", "64",
                               "--false-positive-rate", "0.05",
                               "--filenames", count_filename.get_path().c_str(),
                               "-o", layout_filename.get_path().c_str()};
        seqan3::argument_parser parser{"chopper-layout", static_cast<int>(strlen(*argv)), argv, seqan3::update_notifications::off};
