...
public:
    ...
    size_t get(std::filesystem::path const & filename) const noexcept
    {
        switch (cutoff_kind)
        {
...
private:
    ...
    static constexpr std::array<uint16_t, 4> const cutoffs{1u, 3u, 10u, 20u};
    static constexpr std::array<uint64_t, 4> const cutoff_bounds{314'572'800ULL,
                                                                 524'288'000ULL,
                                                                 1'073'741'824ULL,
                                                                 3'221'225'472ULL};

    uint16_t impl(std::filesystem::path const & filename) const
    {
